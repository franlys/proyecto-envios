    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {Object.entries(datosReporte.resumen).map(([key, value]) => (
          <div key={key} className="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-md">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-700">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Fecha</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Ruta</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Repartidor</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Facturas</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Entregadas</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">% Entrega</th>
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {datosReporte.rutas.map((ruta, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{new Date(ruta.fecha).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white">{ruta.nombre}</td>
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{ruta.empleadoNombre}</td>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Ruta</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Repartidor</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Tipo</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Descripción</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Monto</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {datosReporte.gastos.map((gasto, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{new Date(gasto.fecha).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{gasto.rutaNombre}</td>
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{gasto.empleadoNombre}</td>
                  <td className="px-4 py-3 text-sm">
                    <span className="px-2 py-1 rounded-full text-xs bg-slate-100 dark:bg-slate-700 text-slate-800 dark:text-slate-200">
                      {gasto.tipoGasto}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{gasto.descripcion}</td>
                  <td className="px-4 py-3 text-sm text-right font-medium text-slate-900 dark:text-white">${gasto.monto}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        {datosReporte.gastosPorTipo && (
          <div className="p-4 border-t dark:border-slate-700">
            <h4 className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-3">Gastos por Tipo</h4>
            <div className="space-y-2">
              {Object.entries(datosReporte.gastosPorTipo).map(([tipo, monto]) => (
                <div key={tipo} className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-slate-900 dark:text-white">${monto}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-50 dark:bg-slate-700">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Tracking</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Cliente</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Dirección</th>
                <th className="px-4 py-3 text-center text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Estado</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Ruta</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-slate-500 dark:text-slate-300 uppercase">Monto</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {datosReporte.facturas.map((factura, idx) => (
                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-700">
                  <td className="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white">{factura.codigoTracking || factura.numeroFactura || '-'}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-slate-900 dark:text-slate-100">{factura.rutaNombre || 'Sin asignar'}</td>
                  <td className="px-4 py-3 text-sm text-right text-slate-900 dark:text-white">${factura.monto}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6">
        ...
        <p className="text-slate-600 dark:text-slate-400">Genera reportes detallados del sistema</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-md p-6 mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="w-full md:w-auto px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition disabled:bg-slate-400"
        >
