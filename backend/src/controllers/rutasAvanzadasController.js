...
export const getContenedoresDisponibles = async (req, res) => {
  try {
    ...
    console.log(`✅ ${contenedores.length} contenedores con facturas listas para ruta.`);
    ...
    res.status(500).json({
      success: false,
      message: 'Error al obtener contenedores disponibles',
      error: error.message
    });
  }
};
...
export const getFacturasDisponibles = async (req, res) => {
  try {
    const companyId = req.userData?.companyId;
    const { contenedorId } = req.query; // ✅ Recibir el ID del contenedor
    ...
    console.log(`📦 Obteniendo facturas disponibles para company: ${companyId}`);
    if (contenedorId) {
      console.log(`🔍 Filtrando por contenedor: ${contenedorId}`);
    }

    // Obtener facturas en estado "confirmada_secretaria" que NO tienen rutaId
    let query = db.collection('recolecciones')
      .where('companyId', '==', companyId)
      .where('estado', '==', 'confirmada_secretaria'); // ✅ ESTADO CORREGIDO
      
    // ✅ APLICAR FILTRO DE CONTENEDOR SI EXISTE
    if (contenedorId) {
    ...
    snapshot.forEach(doc => {
      ...
      // Filtrar solo las que NO tienen ruta asignada
      if (!data.rutaId && !data.rutaAsignada) {
        // Inferir sector desde diferentes ubicaciones posibles
        const sector = data.destinatario?.sector || 
                      data.sector || 
                      'Sin Sector';
        
        // Inferir zona desde el mapeo o desde el destinatario
        const zona = SECTOR_A_ZONA[sector] || data.destinatario?.zona || 'Sin Zona';

        facturas.push({
          id: doc.id,
          codigoTracking: data.codigoTracking || doc.id,
          cliente: data.destinatario?.nombre || 
                  data.remitente?.nombre || 
                  'Cliente sin nombre',
          direccion: data.destinatario?.direccion || 'Sin dirección',
          sector: sector,
          zona: zona, // Usar la zona inferida
          monto: data.facturacion?.total || 0,
          items: data.items || [],
    ...
    console.log(`✅ ${facturas.length} facturas disponibles encontradas (estado 'confirmada_secretaria')`);
    ...
    res.status(500).json({
      success: false,
      message: 'Error al obtener facturas disponibles',
      error: error.message
    });
  }
};
...
// ========================================
// CREAR RUTA CON SISTEMA LIFO Y ORDEN
// ========================================
